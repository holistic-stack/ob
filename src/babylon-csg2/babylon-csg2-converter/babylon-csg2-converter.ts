...
import { parseOpenSCADCode, type Result } from '../utils/parser-resource-manager';
import { 
  isCubeNode, 
  isSphereNode, 
  isCylinderNode, 
...
export class BabylonCSG2Converter {
  ...
  private convertSingleNode(node: ASTNode): Result<BABYLON.Mesh | null, string> {
    try {
      // Primitive shapes
      if (isCubeNode(node)) return this.createCube(node);
      if (isSphereNode(node)) return this.createSphere(node);
      if (isCylinderNode(node)) return this.createCylinder(node);

      // CSG operations (these are the complex ones requiring CSG2)
      if (isUnionNode(node)) return this.createUnion(node);
      if (isDifferenceNode(node)) return this.createDifference(node);
      if (isIntersectionNode(node)) return this.createIntersection(node);

      // Transformations (these wrap other operations)
      if (isTranslateNode(node)) return this.createTranslate(node);
      if (isScaleNode(node)) return this.createScale(node);
      if (isRotateNode(node)) return this.createRotate(node);
