...
import { NullEngine, Scene, CreateBox, CreateSphere, Vector3, Matrix } from '@babylonjs/core';
...
describe('Geometric Accuracy Visual Regression Tests', () => {
  ...
  describe('Transformation Accuracy', () => {
    it('should apply translate transformation correctly', async () => {
      ...
      const transformNode = babylonNodes.find(node => 
        node.metadata?.transform === 'translate'
      );
    ...
    it('should apply rotate transformation correctly', async () => {
      ...
      const rotateNode = babylonNodes.find(node => 
        node.metadata?.transform === 'rotate'
      );
    ...
    it('should apply scale transformation correctly', async () => {
      ...
      // Look for scale node
      const scaleNode = babylonNodes.find(node => 
        node.metadata?.transform === 'scale'
      );
  ...
  describe('Boolean Operation Accuracy', () => {
    it('should process union operation with correct structure', async () => {
      ...
      const unionNode = babylonNodes.find(node => 
        node.metadata?.operation === 'union'
      );
    ...
    it('should process difference operation with correct structure', async () => {
      ...
      const differenceNode = babylonNodes.find(node => 
        node.metadata?.operation === 'difference'
      );
    ...
    it('should process intersection operation with correct structure', async () => {
      ...
      const intersectionNode = babylonNodes.find(node => 
        node.metadata?.operation === 'intersection'
      );
  ...
  describe('Complex Geometry Accuracy', () => {
    it('should handle nested transformations correctly', async () => {
      ...
      const transformNodes = babylonNodes.filter(node => 
    ...
    it('should handle complex boolean operations correctly', async () => {
      ...
      const booleanNodes = babylonNodes.filter(node => 
