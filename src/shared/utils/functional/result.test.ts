...
describe('Result Utilities', () => {
  ...
  describe('chain', () => {
    ...
    it('should chain operations that can fail', () => {
      const result = success(5);
      const chained = chain(result, x => x > 3 ? success(x * 2) : error('too small'));
  ...
  describe('unwrap', () => {
    ...
    it('should throw for error results', () => {
      ...
      expect(() => unwrap(result)).toThrow('Attempted to unwrap error Result: test error');
    });
  });
  ...
  describe('filter', () => {
    it('should pass values that match predicate', () => {
      ...
      const filtered = filter(result, x => x > 5, 'too small');
