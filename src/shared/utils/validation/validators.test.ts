  ...
  describe("OpenSCAD validators", () => {
    ...
    describe("openscadVector", () => {
      ...
      it("should fail for invalid vectors", () => {
        expect(validator3D("[1, 2]")).toBe(
          "Vector must have exactly 3 components",
        );
        expect(validator3D("[1, 2, 3, 4]")).toBe(
          "Vector must have exactly 3 components",
        );
  ...
  describe("File validators", () => {
    describe("fileExtension", () => {
      ...
      it("should fail for disallowed extensions", () => {
        expect(validator("document.pdf")).toBe(
          "File must have one of these extensions: jpg, png, gif",
        );
      });
    });
  });
    ...
    describe("validateForm", () => {
      ...
      it("should validate entire form", () => {
        ...
        const result = validateForm(
          ...
          schema as Record<
            string,
            ReadonlyArray<(value: unknown) => string | null>
          >,
        );
        expect(result.success).toBe(true);
      });
