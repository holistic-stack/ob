 ...
 */
export const useMonacoEditor = (options: Partial<UseMonacoEditorOptions> = {}): UseMonacoEditorReturn => {
   ...
   */
  const handleCursorPositionChange = useCallback((e: monaco.editor.ICursorPositionChangedEvent) => {
    const newPosition = {
      line: e.position.lineNumber,
      column: e.position.column
    };
    
    if (newPosition.line !== cursorPosition.line || newPosition.column !== cursorPosition.column) {
   ...
   */
  const handleSelectionChange = useCallback((e: monaco.editor.ICursorSelectionChangedEvent) => {
    const newSelection = e.selection.isEmpty() ? null : {
    ...
    setSelection: (selection) => {
      if (editorRef.current && selection) {
        editorRef.current.setSelection({
          startLineNumber: selection.start,
          startColumn: 1,
          endLineNumber: selection.end,
          endColumn: 1
        });
      }
    },
   ...
   */
  const initializeEditor = useCallback((editor: monaco.editor.IStandaloneCodeEditor) => {
    const result = tryCatch(() => {
      ...
      return editor;
    }, (err) => `Failed to initialize Monaco Editor: ${err instanceof Error ? err.message : String(err)}`);
