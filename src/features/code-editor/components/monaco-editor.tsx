...
import type { MonacoEditorProps, EditorChangeEvent, EditorCursorEvent, EditorSelectionEvent, EditorFocusEvent } from '../types/editor.types';
...
export const MonacoEditorComponent: React.FC<MonacoEditorProps> = ({
  ...
  const handleEditorMount = useCallback((
    ...
    const result = tryCatch(() => {
      ...
      return editor;
    }, (err) => `Failed to mount Monaco Editor: ${err instanceof Error ? err.message : String(err)}`);
  ...
  const handleEditorUnmount = useCallback(() => {
    ...
    }, (err) => `Failed to unmount Monaco Editor: ${err instanceof Error ? err.message : String(err)}`);
