...
export const validateMonacoConfig = (config: any): Result<void, string> => {
  return tryCatch(() => {
    ...
    return undefined;
  }, (err) => `Monaco configuration validation failed: ${err instanceof Error ? err.message : String(err)}`);
};
 ...
 */
export const createViteMonacoPlugin = (
  options: Partial<MonacoEditorConfig> = {}
) => {
...
export const configureMonacoEnvironment = (): Result<void, string> => {
  return tryCatch(() => {
    ...
    return undefined;
  }, (err) => `Failed to configure Monaco environment: ${err instanceof Error ? err.message : String(err)}`);
};
...
export const validateMonacoInstallation = (): Result<void, string> => {
  return tryCatch(() => {
    ...
    return undefined;
  }, (err) => `Monaco Editor installation validation failed: ${err instanceof Error ? err.message : String(err)}`);
};
