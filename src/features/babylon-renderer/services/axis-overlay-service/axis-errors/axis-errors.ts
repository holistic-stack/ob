 ...
 */
export class AxisErrorFactory {
  /**
   * Create an axis creation error
   */
  static createCreationError(
    message: string,
 ...
 */
export class AxisErrorUtils {
  /**
   * Check if an error is an axis error
   */
  static isAxisError(error: unknown): error is AxisError {
    return (
      typeof error === 'object' &&
      error !== null &&
      'code' in error &&
   ...
   */
  static isAxisErrorOfType<T extends AxisError['code']>(
    error: unknown,
    code: T
   ...
   */
  static formatError(error: AxisError): string {
   ...
   */
  static extractContext(error: AxisError): Record<string, unknown> {
    const baseContext = {
      code: error.code,
   ...
   */
  static getUserFriendlyMessage(error: AxisError): string {
    switch (error.code) {
      case 'AXIS_CREATION_ERROR':
 ...
 */
export class AxisResultUtils {
  /**
   * Create a successful result
   */
  static success<T>(data: T): AxisResult<T> {
   ...
   */
  static failure<T>(error: AxisError): AxisResult<T> {
   ...
   */
  static isSuccess<T>(result: AxisResult<T>): result is { success: true; data: T } {
   ...
   */
  static isFailure<T>(result: AxisResult<T>): result is { success: false; error: AxisError } {
    return !result.success;
  }
}
