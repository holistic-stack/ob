...
export const useExport = (scene: Scene | null): UseExportReturn => {
  ...
  const exportMeshes = useCallback(
    async (
      meshes: readonly AbstractMesh[],
      config: ExportConfig,
      onProgress?: ExportProgressCallback
  ...
  const getDefaultConfig = useCallback(
    (format: ExportFormat, filename?: string): ExportConfig => {
      if (!exportService) {
        return {
          format,
...
export const useQuickExport = (scene: Scene | null) => {
  ...
  const exportSTL = useCallback(
    async (meshes: readonly AbstractMesh[], filename?: string) => {
  ...
  const exportSceneSTL = useCallback(
  ...
  const exportSceneGLTF = useCallback(
    async (filename?: string) => {
      const config = getDefaultConfig('gltf', filename);
      return exportScene(config);
    },
