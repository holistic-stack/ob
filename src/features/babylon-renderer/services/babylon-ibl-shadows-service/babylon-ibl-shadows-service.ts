...
export class BabylonIBLShadowsService {
  ...
  async init(scene: Scene, config?: Partial<IBLShadowConfig>): Promise<IBLShadowInitResult> {
    ...
    return tryCatchAsync(async () => {
      ...
      if (!this.isIBLShadowsSupported(scene)) {
        throw this.createError('IBL_NOT_SUPPORTED', 'IBL shadows are not supported in this environment');
      }
      ...
      logger.debug('[DEBUG][BabylonIBLShadowsService] IBL shadows service initialized successfully');
    }, (error) => {
      // If error is already an IBLShadowError, preserve it
      if (error && typeof error === 'object' && 'code' in error) {
        return error as IBLShadowError;
      }
      return this.createError('INITIALIZATION_FAILED', `Failed to initialize IBL shadows: ${error}`);
    });
  }
  ...
  async loadEnvironmentTexture(textureUrl: string): Promise<Result<BaseTexture, IBLShadowError>> {
    ...
    return tryCatchAsync(async () => {
      if (!this.scene) {
        throw this.createError('SCENE_NOT_PROVIDED', 'Scene must be initialized before loading textures');
      }
      ...
      return this.createError('TEXTURE_LOAD_FAILED', `Failed to load environment texture: ${error}`);
    });
  }
  ...
  applyToMesh(mesh: AbstractMesh): IBLShadowApplyResult {
    ...
    return tryCatch(() => {
      if (!this.scene || !this.environmentTexture) {
        throw this.createError('INVALID_CONFIG', 'IBL shadows must be initialized before applying to meshes');
      }
      ...
      return this.createError('MESH_APPLICATION_FAILED', `Failed to apply IBL shadows to mesh: ${error}`);
    });
  }
  ...
  applyToMeshes(meshes: AbstractMesh[]): IBLShadowApplyResult {
    logger.debug(`[DEBUG][BabylonIBLShadowsService] Applying IBL shadows to ${meshes.length} meshes`);
    ...
    return tryCatch(() => {
      ...
      logger.debug(`[DEBUG][BabylonIBLShadowsService] IBL shadows applied to all meshes successfully`);
    }, (error) => {
      // If error is already an IBLShadowError, preserve it
      if (error && typeof error === 'object' && 'code' in error) {
        return error as IBLShadowError;
      }
      return this.createError('MESH_APPLICATION_FAILED', `Failed to apply IBL shadows to meshes: ${error}`);
    });
  }
  ...
  updateConfig(newConfig: Partial<IBLShadowConfig>): IBLShadowUpdateResult {
    ...
    return tryCatch(() => {
      ...
      logger.debug('[DEBUG][BabylonIBLShadowsService] IBL shadow configuration updated successfully');
    }, (error) => this.createError('INVALID_CONFIG', `Failed to update IBL shadow configuration: ${error}`));
  }
