...
export class OpenscadParser {
  ...
  parseCST(code: string): TreeSitter.Tree | null {
    ...
    try {
      // Always use cold parsing for consistent, reliable results
  ...
  parseAST(code: string): ASTNode[] {
    try {
      ...
      return this.applyGrammarWorkarounds(code, ast);
    } catch (error) {
  ...
  /**
   * Parse an individual statement using Tree-sitter.
   */
  private parseIndividualStatement(statement: string, startLine: number): ASTNode[] {
    try {
      ...
      // Process nodes for this individual statement
  ...
  /**
   * Process nodes at the statement level.
   */
  ...
  parseASTWithResult(code: string): Result<ASTNode[], string> {
    try {
      ...
      // Process the AST
      const processedAST = this.applyGrammarWorkarounds(code, ast);
      return { success: true, data: processedAST };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.errorHandler.handleError(new Error(`Failed to generate AST: ${errorMessage}`));
