...
describe('extractValue - Vector Literals Regression Tests', () => {
  ...
  describe('Memory Management and Node Text Extraction', () => {
    it('should extract vectors without relying on potentially truncated node.text', () => {
      // This test specifically addresses the Tree-sitter memory management issues
      // mentioned in the value-extractor.ts file
      const code = 'cube([10, 10, 10]);';
      const tree = parser.parse(code);
      ...
      // Ensure the extraction doesn't produce truncated or null values
      (result as number[]).forEach((val) => {
        expect(val).toBe(10);
        expect(Number.isFinite(val)).toBe(true);
      });
    ...
    it('should handle long source code without truncation in vector extraction', () => {
      const longPrefix = '// '.repeat(100); // Create a long comment prefix
      const code = `${longPrefix}\ncube([10, 10, 10]);`;
