...
export class ForLoopVisitor extends BaseASTVisitor {
  ...
  override visitForStatement(node: TSNode): ast.ForLoopNode | ast.ErrorNode | null {
    this.errorHandler.logInfo(
      '[ForLoopVisitor.visitForStatement] Method Enter',
      'ForLoopVisitor.visitForStatement: enter',
      node
    );
    ...
    if (
      !bodyCstNode &&
      node.lastNamedChild &&
      !potentialAssignmentsCstNodes.includes(node.lastNamedChild)
    ) {
    ...
    if (potentialAssignmentsCstNodes.length > 0) {
      ...
      for (let i = 0; i < potentialAssignmentsCstNodes.length; i++) {
        ...
        if (!child) {
          this.errorHandler.logError(
            `Null CST child encountered at index ${i} when searching for for_assignment or body.`,
            'ForLoopVisitor.visitForStatement: null_child_in_potential_assignments',
            node
          );
          continue;
        }
        ...
        if (child.type === 'for_assignment') {
          ...
          const iteratorAstNode =
            this.expressionVisitor.dispatchSpecificExpression(iteratorCstNode);
      ...
      if (!bodyCstNode) {
        ...
        if (directBodyCstNode) {
          ...
          node.lastNamedChild &&
          node.lastNamedChild !== iteratorCstNode &&
          node.lastNamedChild !== rangeCstNode
        ) {
      ...
      if (!iteratorCstNode || !rangeCstNode) {
        ...
        return createErrorNodeInternal(
          node,
          'Direct assignment in for_statement is missing iterator or range.',
          'E_FOR_DIRECT_ASSIGN_MISSING_PARTS',
          node.type,
          node.text
        );
      }
      ...
      if (!iteratorAstNode) {
        this.errorHandler.logError(
          'Failed to visit iterator AST node for direct assignment.',
          'ForLoopVisitor.visitForStatement: visit_failed_iterator_direct_assign',
          iteratorCstNode
        );
        return createErrorNodeInternal(
      ...
      if (!rangeAstNode) {
        this.errorHandler.logError(
          'Failed to visit range AST node for direct assignment.',
          'ForLoopVisitor.visitForStatement: visit_failed_range_direct_assign',
          rangeCstNode
        );
        return createErrorNodeInternal(
      ...
      if (isAstVariableNode(iteratorAstNode)) {
        ...
        this.errorHandler.logError(
          `Expected variable for loop variable (direct assign), got ${iteratorAstNode.type}.`,
          'ForLoopVisitor.visitForStatement: non_variable_iterator_direct_assign',
          iteratorCstNode
        );
        return createErrorNodeInternal(
          iteratorCstNode as TSNode,
      ...
      if (isAstExpressionNode(rangeAstNode)) {
        ...
        this.errorHandler.logError(
          `Expected expression for loop range (direct assign), got ${rangeAstNode.type}.`,
          'ForLoopVisitor.visitForStatement: non_expression_range_direct_assign',
          rangeCstNode
        );
        return createErrorNodeInternal(
          rangeCstNode as TSNode,
    ...
    if (loopVariablesAst.length === 0) {
      this.errorHandler.logError(
        `No valid loop assignments found in for_statement. CST Node: ${node.text}`,
        'ForLoopVisitor.visitForStatement: no_valid_assignments_found',
        node
      );
      return createErrorNodeInternal(
        node,
        'No assignments in for_statement',
        'E_FOR_NO_ASSIGNMENTS',
        node.type,
        node.text
      );
    }
    ...
    if (bodyCstNode) {
      ...
      if (bodyCstNode.type === 'block') {
        ...
        if (visitedBodyAstNodes && visitedBodyAstNodes.length > 0) {
          bodyAstNodes = visitedBodyAstNodes.flatMap((stmt) =>
            stmt && stmt.type !== 'error' ? [stmt as ast.StatementNode] : []
          );
          const bodyErrors = visitedBodyAstNodes.filter((stmt) => stmt?.type === 'error');
          if (bodyErrors.length > 0) {
            this.errorHandler.logWarning(
              `Found ${bodyErrors.length} error(s) in for loop body.`,
              'ForLoopVisitor.visitForStatement: errors_in_body',
              bodyCstNode
            );
          }
        } else {
          this.errorHandler.logWarning(
            'For loop body visitation returned empty or null.',
            'ForLoopVisitor.visitForStatement: body_visit_returned_empty',
            bodyCstNode
          );
          bodyAstNodes = [];
        }
        ...
        if (visitedBodyAstNode) {
          if (visitedBodyAstNode.type !== 'error') {
            ...
            this.errorHandler.logWarning(
              'For loop body statement resulted in an error.',
              'ForLoopVisitor.visitForStatement: body_statement_error',
              bodyCstNode
            );
            bodyAstNodes = [];
          }
          ...
          this.errorHandler.logWarning(
            'For loop body statement visitation returned null.',
            'ForLoopVisitor.visitForStatement: body_statement_null',
            bodyCstNode
          );
          bodyAstNodes = [];
        }
      ...
      this.errorHandler.logWarning(
        'No body CST node found for for_statement. Assuming empty body.',
        'ForLoopVisitor.visitForStatement: no_body_found_assuming_empty',
        node
      );
    ...
    if (loopVariablesAst.length !== loopRangesAst.length) {
      this.errorHandler.logError(
        `Internal parsing error: Mismatch between parsed loop variables (${loopVariablesAst.length}) and ranges (${loopRangesAst.length}).`,
        'ForLoopVisitor.visitForStatement: variable_range_mismatch',
        node
      );
      return createErrorNodeInternal(
        node,
        'Internal error: Mismatch in parsed loop variables and ranges.',
        'E_FOR_INTERNAL_VAR_RANGE_MISMATCH'
      );
    }
    ...
    for (let i = 0; i < loopVariablesAst.length; i++) {
      ...
      if (typeof variableNode === 'undefined' || typeof rangeNode === 'undefined') {
        ...
        return createErrorNodeInternal(
          node,
          `Internal error: undefined variable/range at index ${i}`,
          'E_FOR_INTERNAL_UNDEFINED_FINAL_ASSEMBLY'
        );
      }

      if (variableNode.type === 'error') {
        this.errorHandler.logError(
          'Encountered pre-existing error node for a loop variable.',
          'ForLoopVisitor.visitForStatement: error_node_as_variable',
          node
        );
      ...
      if (rangeNode.type === 'error') {
        this.errorHandler.logError(
          'Encountered pre-existing error node for a loop range.',
          'ForLoopVisitor.visitForStatement: error_node_as_range',
          node
        );
      ...
      let stepNode: ast.ExpressionNode | ast.ErrorNode | undefined;
    ...
    this.errorHandler.logInfo(
      `[ForLoopVisitor.visitForStatement] Successfully parsed for_statement. Assignments: ${finalLoopAssignments.length}, Body statements: ${bodyAstNodes.length}`,
      'ForLoopVisitor.visitForStatement: success',
      node
    );
    return {
      type: 'for_loop',
  ...
  createForNode(node: TSNode, args: ast.Parameter[]): ast.ForLoopNode | ast.ErrorNode | null {
    ...
    if (args.length === 0) {
      this.errorHandler.logWarning(
        'createForNode called with no arguments.',
        'ForLoopVisitor.createForNode: no_arguments',
        node
      );
      return null;
    }
    ...
    for (const arg of args) {
      if (!arg.name) {
        this.errorHandler.logWarning(
          'Argument missing name in createForNode.',
          'ForLoopVisitor.createForNode: arg_missing_name',
          node
        );
        continue;
      }
      ...
      if (typeof arg.name === 'string') {
        this.errorHandler.logWarning(
          `Parameter name '${arg.name}' is a string in createForNode. Creating synthetic VariableNode.`,
          'ForLoopVisitor.createForNode: synthetic_variable_from_string_name',
          node
        );
        variableNode = {
          type: 'variable',
        ...
        const argNameType =
          arg.name && typeof arg.name === 'object'
            ? (arg.name as ast.VariableNode).type
            : typeof arg.name;
        this.errorHandler.logError(
          `Parameter name for for-loop variable is not a VariableNode. Type: ${argNameType}`,
          'ForLoopVisitor.createForNode: invalid_param_name_type',
          node
        );
      ...
      if (typeof arg.value === 'object' && arg.value !== null && 'type' in arg.value) {
        ...
        if (isAstExpressionNode(astValueNode)) {
          ...
          const varNameForLog1 = isAstVariableNode(variableNode)
            ? variableNode.name
            : 'unknown_or_error_variable';
          this.errorHandler.logError(
            `Unexpected ASTNode type for argument '${varNameForLog1}' value: ${unknownTypeString}`,
            'ForLoopVisitor.createForNode: unexpected_arg_value_type',
            node
          );
          rangeNode = createErrorNodeInternal(
            node,
            `Unexpected ASTNode type for argument value: ${unknownTypeString}`,
            'E_UNEXPECTED_ARG_TYPE',
            unknownTypeString
          );
        }
      } else if (
        ...
        const varNameForLog2 = isAstVariableNode(variableNode)
          ? variableNode.name
          : 'unknown_or_error_variable';
        this.errorHandler.logWarning(
          `Primitive value for argument '${varNameForLog2}' in createForNode requires stubbed literal creation.`,
          'ForLoopVisitor.createForNode: stubbed_numeric_literal_primitive',
          node
        );
        rangeNode = createErrorNodeInternal(
          node,
          `Parsing for primitive literal '${arg.value}' is stubbed.`,
          'E_STUBBED_NUMERIC_LITERAL'
        );
      } else if (Array.isArray(arg.value) && arg.value.every((item) => typeof item === 'number')) {
        const varNameForLog3 = isAstVariableNode(variableNode)
          ? variableNode.name
          : 'unknown_or_error_variable';
        this.errorHandler.logWarning(
          `Vector value for argument '${varNameForLog3}' in createForNode requires stubbed literal creation.`,
          'ForLoopVisitor.createForNode: stubbed_numeric_literal_vector',
          node
        );
        rangeNode = createErrorNodeInternal(
          node,
          `Parsing for vector literal '[${arg.value.join(', ')}]' is stubbed.`,
          'E_STUBBED_NUMERIC_LITERAL_VECTOR'
        );
      } else {
        const varNameForLog4 = isAstVariableNode(variableNode)
          ? variableNode.name
          : 'unknown_or_error_variable';
        this.errorHandler.logError(
          `Unhandled type for argument '${varNameForLog4}' value: ${typeof arg.value}`,
          'ForLoopVisitor.createForNode: unhandled_arg_value_type',
          node
        );
        rangeNode = createErrorNodeInternal(
          node,
          `Unhandled argument value type: ${typeof arg.value}`,
          'E_UNHANDLED_ARG_VALUE_TYPE',
          typeof arg.value
        );
      }
      ...
      if (variableNode.type === 'error') {
        this.errorHandler.logError(
          'Skipping assignment due to error in variable.',
          'ForLoopVisitor.createForNode: error_in_variable_skip_push',
          node
        );
        // Optionally, add this error variable to a list of errors for this for_loop creation attempt
      } else if (rangeNode.type === 'error') {
        this.errorHandler.logError(
          'Skipping assignment due to error in range.',
          'ForLoopVisitor.createForNode: error_in_range_skip_push',
          node
        );
    ...
    if (variables.some((v) => 'errorCode' in v.iterable)) {
      ...
      return createErrorNodeInternal(
        node,
        'Failed to process arguments for createForNode due to internal errors.',
        'E_FOR_CREATE_NODE_ARG_PROCESSING'
      );
    }

    if (variables.length === 0) {
      this.errorHandler.logError(
        'No valid variables could be processed by createForNode.',
        'ForLoopVisitor.createForNode: no_variables_processed',
        node
      );
      return null;
    }
