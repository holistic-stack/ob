...
describe('EchoStatementVisitor', () => {
  ...
  describe('Basic Echo Statements', () => {
    it('should parse basic echo statement with string literal', async () => {
      ...
      expect(ast[0]?.type).toBe('echo_statement');
    ...
    it('should parse echo statement with number literal', async () => {
      ...
      expect(echoNode.arguments[0]).toBeDefined();
      expect(echoNode.arguments[0]?.expressionType).toBe('literal');
      expect((echoNode.arguments[0] as LiteralNode).value).toBe(42);
    });
    ...
    it('should parse echo statement with boolean literal', async () => {
      ...
      expect(echoNode.arguments[0]).toBeDefined();
      expect(echoNode.arguments[0]?.expressionType).toBe('literal');
      expect((echoNode.arguments[0] as LiteralNode).value).toBe(true);
    });
  ...
  describe('Multiple Arguments', () => {
    it('should parse echo statement with multiple string arguments', async () => {
      ...
      expect(echoNode.arguments[0]?.expressionType).toBe('literal');
      expect((echoNode.arguments[0] as LiteralNode).value).toBe('Hello');
      expect((echoNode.arguments[1] as LiteralNode).value).toBe('World');
    });
    ...
    it('should parse echo statement with mixed argument types', async () => {
      ...
      expect(echoNode.arguments[0]?.expressionType).toBe('literal');
      expect((echoNode.arguments[0] as LiteralNode).value).toBe('Value:');
  ...
  describe('Edge Cases', () => {
    ...
    it('should parse echo statement without semicolon', async () => {
      ...
      expect(echoNode.arguments).toHaveLength(1);
      expect(echoNode.arguments[0]?.expressionType).toBe('literal');
      expect((echoNode.arguments[0] as LiteralNode).value).toBe('test');
    });
    ...
    it('should parse multiple echo statements', async () => {
      ...
      expect(ast[0]?.type).toBe('echo_statement');
      expect(ast[1]?.type).toBe('echo_statement');
      expect(ast[2]?.type).toBe('echo_statement');
      ...
      expect((firstEcho.arguments[0] as LiteralNode).value).toBe('First');
      expect((secondEcho.arguments[0] as LiteralNode).value).toBe('Second');
      expect((thirdEcho.arguments[0] as LiteralNode).value).toBe('Third');
    });
  });
