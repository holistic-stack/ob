...
describe('OpenSCADSyntaxError', () => {
  ...
  it('should create a SyntaxError with the correct properties', () => {
    const error = new OpenSCADSyntaxError(message, source, position);

    expect(error).toBeInstanceOf(Error);
    expect(error).toBeInstanceOf(OpenSCADSyntaxError);
  ...
  it('should create a missing token error', () => {
    const tokenName = ']';
    const error = OpenSCADSyntaxError.missingToken(tokenName, source, position);
  ...
  it('should create an unexpected token error', () => {
    const foundToken = ')';
    const expectedToken = ']';
    const error = OpenSCADSyntaxError.unexpectedToken(foundToken, expectedToken, source, position);
  ...
  it('should create an unmatched token error', () => {
    const openToken = '[';
    const closeToken = ']';
    const error = OpenSCADSyntaxError.unmatchedToken(openToken, closeToken, source, position);
  ...
  it('should create a missing semicolon error', () => {
    const error = OpenSCADSyntaxError.missingSemicolon(source, position);
