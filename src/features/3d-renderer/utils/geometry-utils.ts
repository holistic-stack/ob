 ...
 */
export const createPolygon = (vertices: VertexData[], shared: any): PolygonData => {
...
export const classifyPolygonToPlane = (
  ...
  for (const vertex of polygon.vertices) {
    ...
    switch (classification) {
      ...
      case 'front': type = front; break;
      case 'coplanar': type = coplanar; break;
      default: type = coplanar;
    }
  ...
  switch (polygonType) {
    ...
    case spanning: {
      ...
      for (let i = 0; i < polygon.vertices.length; i++) {
        ...
        if ((ti | tj) === spanning) {
          const t = (plane.w - plane.normal.dot(vi.pos)) /
...
export const isValidVertex = (vertex: VertexData): boolean => {
  return vertex.pos && vertex.normal && vertex.uv &&
    Number.isFinite(vertex.pos.x) && Number.isFinite(vertex.pos.y) && Number.isFinite(vertex.pos.z) &&
    Number.isFinite(vertex.normal.x) && Number.isFinite(vertex.normal.y) && Number.isFinite(vertex.normal.z);
};

export const isValidPlane = (plane: PlaneData): boolean => {
  return plane.normal && Number.isFinite(plane.w) &&
