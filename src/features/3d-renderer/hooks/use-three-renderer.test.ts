...
vi.mock('../../store', () => {
  ...
  const mockSelectRenderingState = vi.fn(() => ({
    isRendering: false,
    meshes: [],
    renderErrors: [],
  }));
  const mockSelectPerformanceMetrics = vi.fn(() => ({
    renderTime: 0,
    parseTime: 0,
    memoryUsage: 0,
  }));
  ...
  return {
    useAppStore: vi.fn((selector) => {
      ...
      if (typeof selector === 'function') {
        ...
        if (selectorName === 'selectRenderingCamera')
          return { position: [5, 5, 5], target: [0, 0, 0] };
        if (selectorName === 'selectRenderingState')
          return { isRendering: false, meshes: [], renderErrors: [] };
        if (selectorName === 'selectPerformanceMetrics')
          return { renderTime: 0, parseTime: 0, memoryUsage: 0 };
