...
describe('MatrixTelemetryService', () => {
  ...
  describe('Performance Regression Detection', () => {
    ...
    it('should not detect regressions for operations within baseline', () => {
      console.log('[DEBUG][MatrixTelemetryServiceTest] Testing no regression for normal operations');
  ...
  describe('Report Generation', () => {
    ...
    it('should generate appropriate recommendations', () => {
      ...
      expect(report.recommendations.some(r => 
        r.includes('regression') || r.includes('failure rate')
      )).toBe(true);
    });
  ...
  describe('Logging and Debugging', () => {
    ...
    it('should not log operations when debugging is disabled', () => {
      ...
      expect(consoleSpy).not.toHaveBeenCalledWith(
        expect.stringContaining('add SUCCESS in 5ms')
      );
