...
export class MatrixTelemetryService {
  ...
  private generateRegressionRecommendations(
    ...
    switch (operation) {
      ...
      case 'inverse':
        ...
        recommendations.push('Check matrix condition number - use pseudo-inverse for ill-conditioned matrices');
        break;
  ...
  private trackPerformanceRegression(regression: PerformanceRegression): void {
    ...
    console.log(`[DEBUG][MatrixTelemetryService] Tracking regression: ${regression.operation} (${regression.severity})`);
  }
  ...
  generateReport(timeRange?: readonly [number, number]): TelemetryReport {
    ...
    const averageExecutionTime = successfulOps.reduce((sum, op) => sum + op.duration, 0) / successfulOps.length;
  ...
  private generateRecommendations(
    ...
    if (regressions.length > 0) {
      recommendations.push(`${regressions.length} performance regression(s) detected - investigate immediately`);
    }
    ...
    const avgExecutionTime = operations
      .filter(op => op.success)
      .reduce((sum, op) => sum + op.duration, 0) / operations.filter(op => op.success).length;
    
    if (avgExecutionTime > this.deps.config.performance.performanceThreshold) {
      recommendations.push('Average execution time exceeds performance threshold - consider optimization');
    }
