 ...
 */
const parserFriendlyRadius = () => fc.oneof(
  fc.float({ min: Math.fround(0.1), max: Math.fround(20.0), noNaN: true }).map(n => Math.round(n * 10) / 10), // One decimal place
  fc.float({ min: Math.fround(0.5), max: Math.fround(15.0), noNaN: true }).map(n => Math.round(n * 100) / 100), // Two decimal places
  fc.integer({ min: 1, max: 20 }) // Integer values
);

const parserFriendlySize = () => fc.oneof(
  fc.integer({ min: 1, max: 50 }), // Integer sizes
  fc.float({ min: Math.fround(1.0), max: Math.fround(50.0), noNaN: true }).map(n => Math.round(n * 10) / 10), // One decimal place
  fc.float({ min: Math.fround(1.0), max: Math.fround(25.0), noNaN: true }).map(n => Math.round(n * 100) / 100) // Two decimal places
);

const parserFriendlySizeVector = () => fc.tuple(
  parserFriendlySize(),
  parserFriendlySize()
);

const parserFriendlyPolygonPoints = () => fc.array(
  fc.tuple(
    fc.float({ min: Math.fround(-10.0), max: Math.fround(10.0), noNaN: true }).map(n => Math.round(n * 10) / 10),
    fc.float({ min: Math.fround(-10.0), max: Math.fround(10.0), noNaN: true }).map(n => Math.round(n * 10) / 10)
  ),
...
describe('AST to CSG Converter - 2D Operations Fuzzy Testing', () => {
  ...
  describe('Polygon Parameter Extraction Validation', () => {
    ...
    it('should handle simple triangle polygon operations', () => {
      fc.assert(
        fc.property(
          fc.constant([[0, 0], [5, 0], [2.5, 4]]), // Simple triangle
          (points) => {
            const pointsStr = points.map(([x, y]) => `[${x}, ${y}]`).join(', ');
            const code = `polygon([${pointsStr}]);`;
            const extracted = extractPolygonParameters(code);
          ...
          async (radius) => {
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.warn(`Conversion failed for circle(r=${radius}): ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
          ...
          async (size) => {
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.warn(`Conversion failed for square(${size}): ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
          ...
          async (points) => {
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.warn(`Conversion failed for polygon([${pointsStr}]): ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (operations) => {
            ...
            const codeLines = operations.map(([type, params]) => {
              if (type === 'circle') {
                return `circle(r=${params});`;
              } else if (type === 'square') {
                return `square(${params});`;
              } else if (type === 'polygon') {
                const pointsStr = (params as Array<[number, number]>).map(([x, y]) => `[${x}, ${y}]`).join(', ');
            ...
            try {
              ...
              for (let i = 0; i < operations.length; i++) {
                const [expectedType] = operations[i]!;
                const node = ast[i];
        ...
        fc.asyncProperty(
          ...
          async (points) => {
            ...
            try {
              ...
              if (result.success) {
                expect(conversionTime).toBeLessThan(100); // Performance target
                logger.debug(`Polygon([${pointsStr}]) conversion time: ${conversionTime.toFixed(2)}ms`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
