 ...
 */
const parserFriendlyHeight = () => fc.oneof(
  fc.integer({ min: 1, max: 50 }), // Integer heights
  fc.float({ min: 1.0, max: 50.0, noNaN: true }).map(n => Math.round(n * 10) / 10), // One decimal place
  fc.float({ min: 1.0, max: 25.0, noNaN: true }).map(n => Math.round(n * 100) / 100) // Two decimal places
);

const parserFriendlyAngle = () => fc.oneof(
  fc.integer({ min: 1, max: 360 }), // Integer degrees
  fc.float({ min: 1.0, max: 360.0, noNaN: true }).map(n => Math.round(n * 10) / 10), // One decimal place
  fc.float({ min: 1.0, max: 180.0, noNaN: true }).map(n => Math.round(n * 100) / 100) // Two decimal places
);

const parserFriendlyTwist = () => fc.oneof(
  fc.integer({ min: -180, max: 180 }), // Integer twist degrees
  fc.float({ min: -90.0, max: 90.0, noNaN: true }).map(n => Math.round(n * 10) / 10) // One decimal place
);

const parserFriendlyScale = () => fc.oneof(
...
const parserFriendly2DShape = () => fc.oneof(
        ...
        fc.asyncProperty(
          ...
          async (height, shape) => {
            ...
            if (!parseResult.success) {
              logger.warn(`Parse failed for linear_extrude(height=${height}) ${shape}: ${parseResult.error}`);
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.debug(`✅ Linear_extrude(height=${height}) ${shape} applied correctly to mesh`);
              } else {
                logger.warn(`Conversion failed for linear_extrude(height=${height}) ${shape}: ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (height, twist, shape) => {
            ...
            logger.debug(`Testing linear_extrude(height=${height}, twist=${twist}) ${shape} conversion`);
            
            // Parse the code
            const parseResult = await parserService.parseDocument(code);
            if (!parseResult.success) {
              logger.warn(`Parse failed for linear_extrude(height=${height}, twist=${twist}) ${shape}: ${parseResult.error}`);
              return; // Skip this test case
            }
            
            const ast = parseResult.data.ast;
            if (!ast || ast.length === 0) {
              logger.warn(`No AST nodes for linear_extrude(height=${height}, twist=${twist}) ${shape}`);
              return; // Skip this test case
            }
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.debug(`✅ Linear_extrude(height=${height}, twist=${twist}) ${shape} applied correctly to mesh`);
              } else {
                logger.warn(`Conversion failed for linear_extrude(height=${height}, twist=${twist}) ${shape}: ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (angle, shape) => {
            ...
            const parseResult = await parserService.parseDocument(code);
            if (!parseResult.success) {
              logger.warn(`Parse failed for rotate_extrude(angle=${angle}) ${shape}: ${parseResult.error}`);
              return; // Skip this test case
            }
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.warn(`Conversion failed for rotate_extrude(angle=${angle}) ${shape}: ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
          ...
          async (shape) => {
            ...
            try {
              ...
              if (result.success && result.data.mesh) {
                ...
                logger.debug(`✅ Rotate_extrude() (full revolution) ${shape} applied correctly to mesh`);
              } else {
                logger.warn(`Conversion failed for rotate_extrude() ${shape}: ${result.success ? 'no mesh' : result.error}`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (operations) => {
            ...
            const codeLines = operations.map(([height, shape], i) =>
              `linear_extrude(height=${height}) ${shape};`
            );
            ...
            try {
              ...
              for (let i = 0; i < operations.length; i++) {
                const [expectedHeight, expectedShape] = operations[i]!;
                const node = ast[i];
                ...
                if (result.success && result.data.mesh) {
                  ...
                  logger.debug(`✅ Linear_extrude(height=${expectedHeight}) ${expectedShape} operation ${i} applied correctly`);
                }
              }
            } finally {
              clearSourceCodeForExtraction();
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (height, shape) => {
            ...
            const parseResult = await parserService.parseDocument(code);
            if (!parseResult.success) {
              logger.warn(`Parse failed for performance test with linear_extrude(height=${height}) ${shape}`);
              return; // Skip this test case
            }
            ...
            try {
              ...
              if (result.success) {
                ...
                logger.debug(`Linear_extrude(height=${height}) ${shape} conversion time: ${conversionTime.toFixed(2)}ms`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (angle, shape) => {
            ...
            const parseResult = await parserService.parseDocument(code);
            if (!parseResult.success) {
              logger.warn(`Parse failed for performance test with rotate_extrude(angle=${angle}) ${shape}`);
              return; // Skip this test case
            }
            ...
            try {
              ...
              if (result.success) {
                expect(conversionTime).toBeLessThan(100); // Performance target
                logger.debug(`Rotate_extrude(angle=${angle}) ${shape} conversion time: ${conversionTime.toFixed(2)}ms`);
              }
            } finally {
              clearSourceCodeForExtraction();
            }
