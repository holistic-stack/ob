...
describe('Manifold Material Manager', () => {
  ...
  describe('Material Mapping', () => {
    ...
    it('should handle material mapping updates', () => {
      ...
      const initialMaterials = [{ id: 'material1', name: 'Red Material' }];
      ...
      if (initialMapping.success) {
        // Add new material
        const updatedMaterials = [...initialMaterials, { id: 'material2', name: 'Blue Material' }];

        const updatedMapping = createMaterialMapping(
          updatedMaterials,
          reservedRange,
          initialMapping.data
        );
