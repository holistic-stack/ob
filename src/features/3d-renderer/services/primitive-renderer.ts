...
export const createPrimitiveRendererFactory = (): PrimitiveRendererFactory => ({
  ...
  createSphere: (radius: number, segments = 32) => {
    ...
    }, (err) => `Failed to create sphere geometry: ${err instanceof Error ? err.message : String(err)}`);
  },
  ...
  createCylinder: (radius: number, height: number, segments = 32) => {
    ...
    }, (err) => `Failed to create cylinder geometry: ${err instanceof Error ? err.message : String(err)}`);
  },

  createPolyhedron: (vertices: ReadonlyArray<readonly [number, number, number]>, faces: ReadonlyArray<ReadonlyArray<number>>) => {
    return tryCatch(() => {
      ...
      return geometry;
    }, (err) => `Failed to create polyhedron geometry: ${err instanceof Error ? err.message : String(err)}`);
  }
});
...
export const createMaterialFactory = (): MaterialFactory => ({
  createStandard: (config: MaterialConfig) => {
    return tryCatch(() => {
      ...
      return material;
    }, (err) => `Failed to create standard material: ${err instanceof Error ? err.message : String(err)}`);
  },
  ...
  createWireframe: (color: string) => {
    ...
    }, (err) => `Failed to create wireframe material: ${err instanceof Error ? err.message : String(err)}`);
  },
  ...
  createTransparent: (color: string, opacity: number) => {
    ...
    }, (err) => `Failed to create transparent material: ${err instanceof Error ? err.message : String(err)}`);
  }
});
  ...
  return tryCatch(() => {
    transformations.forEach(transform => {
      switch (transform.type) {
        case 'translate': {
          const [x = 0, y = 0, z = 0] = transform.parameters.vector as [number, number, number] || [0, 0, 0];
  ...
  return tryCatch(() => {
    ...
    switch (params.type) {
      case 'cube': {
        ...
        geometryResult = geometryFactory.createCube(size as number | readonly [number, number, number]);
        break;
      }
      case 'sphere': {
