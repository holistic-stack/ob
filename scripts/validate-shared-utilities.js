...
function validateErrorHandlingPatterns(content, filePath) {
  ...
  const manualErrorPattern = /{\s*message:\s*[^,]+,\s*line:\s*\d+,\s*column:\s*\d+,\s*severity:\s*['"]error['"]?\s*}/g;
 ...
 */
function validateResultTypes(content, filePath) {
  const errors = [];
  let warnings = 0;
 ...
 */
function validateJSDocComments(content, filePath) {
  const errors = [];
  let warnings = 0;
